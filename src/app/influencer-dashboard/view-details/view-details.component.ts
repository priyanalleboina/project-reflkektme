...
export class ViewDetailsComponentProduct implements OnInit, OnChanges {
  ...
  constructor(
    ...
    private InfluencerDashboard: InfluencerDashboardComponent
  ) {
  ...
  getProduct() {
    ...
    this.adminDashboardService.findProductByMedia(this.mediaId).subscribe(
      (res: any) => {
        if (res.status === 200) {
          if (res.data) {
    ...
    this.adminDashboardService.createProduct(this.productDetails).subscribe(
      (res: any) => {
        ...
        if (res.status === 200) {
          ...
          this.InfluencerDashboard.getAllProducts();
        }
      },
      (error) => {
        this.toastrService.error(JSON.stringify(error));
