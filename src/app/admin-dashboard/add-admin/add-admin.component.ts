...
export class AddAdminComponent implements OnInit {
  ...
  saveAddAdminDetails() {
    ...
    this.adminDashboardService.saveAddAdminDetails(this.InfluencerDetails).subscribe((result) => {
      // this.InfluencerDetails = {};
      if (result.status === 200) {
        this.toastrService.success("Admin created successfully and Please check your mail");
        this.InfluencerDetails = {};
  ...
  uploadFile(event: any) {
    ...
    if (ext === 'jpg' || ext === 'png' || ext === 'jpeg') {
      this.getImageSize(event.target.files[0], (data) => {
        console.log(data + '===' + (data.width > 600) + '----' + (data.height > 400));
