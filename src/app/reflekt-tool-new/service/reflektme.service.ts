...
export class ReflektmeService {
  ...
  public checkEcommerceToken(token): Observable<any> {
    let params = new HttpParams();
    params = params.append('token', token);
    const url = environment.copmonentUrl.reflektTool.ReflektmeToolController.checkEcommerceToken;
    return this.httpService.get(url,{params: params});
  }
  public login(data): Observable<any> {
    let params = new HttpParams();
    const url = environment.copmonentUrl.reflektTool.ReflektmeToolController.customerLogin;
    return this.httpService.post(url, data);
  }
}
